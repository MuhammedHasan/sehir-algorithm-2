...
class GraphTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.graph_check = Graph.from_checkpoint_graph(self.G_check)
    ...
    def test_from_negative_graph(self):
        neg_graph = Graph.from_negative_graph(self.G_neg)
    ...
    def test_from_checkpoint_graph(self):
        check_graph = Graph.from_checkpoint_graph(self.G_check)
        self.assertEqual(check_graph[1, 2][2, 2], 6)
        self.assertEqual(check_graph[1, 1][2, 1], 0)

    def test_find_sortest_path_for_checkpoints(self):
        to_node = len(self.G) - 1, len(self.G[0]) - 1
        from_node = (0, 0)
        dist, path = self.graph_check.find_sortest_path_for_checkpoints(
            from_node, to_node)
